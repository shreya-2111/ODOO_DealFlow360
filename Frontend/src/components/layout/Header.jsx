...
// Global application header with navigation breadcrumbs, persona switcher, and search
export function Header({ onToggleSidebar }) {
  ...
  // Map route path to human-readable breadcrumb section and title
  const getBreadcrumb = () => {
  ...
  // Switch demo persona and navigate to corresponding screen
  ...
  // Helper to get formatted corporate email for active persona
  const getEmailForRole = (roleId) => {
    if (roleId === 'sales_rep') return 'amit.sharma@dealflow360.in';
    if (roleId === 'sales_manager') return 'priya.patel@dealflow360.in';
    if (roleId === 'finance_ops') return 'rajesh.verma@dealflow360.in';
    if (roleId === 'customer') return 'vikram.malhotra@acme.corp';
    if (roleId === 'admin') return 'neha.gupta@dealflow360.in';
    return 'user@dealflow360.in';
  };

  // Sign out and redirect to login screen
  const handleSignOut = () => {
    setProfileMenuOpen(false);
    addToast('Signed out of DealFlow360. Returning to login...', 'info', 2500);
    navigate('/login');
  };

  // Trigger demo search notification
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 lg:px-6 flex items-center justify-between gap-4">
      {/* Mobile toggle, brand logo, and breadcrumb */}
      ...
      {/* Global search, persona switcher, notifications, and avatar */}
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center p-0.5 rounded-full hover:ring-2 hover:ring-brand-500 transition-all focus:outline-none focus:ring-2 focus:ring-brand-500"
          ...
          {profileMenuOpen && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-72 bg-white rounded-2xl shadow-modal border border-slate-200 z-40 py-2 animate-in fade-in zoom-in-95 duration-150 divide-y divide-slate-100">
                ...
                <div className="p-3.5 flex items-center gap-3">
                  <div className="relative shrink-0">
                    <img
                      src={currentUser.avatar}
                      alt={currentUser.name}
                      className="w-11 h-11 rounded-full object-cover border-2 border-brand-500 shadow-xs"
                    />
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-slate-500 truncate mt-0.5">
                      {getEmailForRole(currentUser.id)}
                    </div>
                    <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-brand-50 text-brand-700 border border-brand-200">
                      {currentUser.role}
                    </span>
                  </div>
                </div>
                ...
                <div className="px-3.5 py-2 bg-slate-50/70 text-[11px] text-slate-600 flex items-center justify-between">
                  <span className="text-slate-500">Department / Team:</span>
                  <span className="font-bold text-slate-800">{currentUser.team}</span>
                </div>
                ...
                <div className="p-1 space-y-0.5">
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"
                  >
                  ...
                  <button
                    onClick={() => {
                      setProfileMenuOpen(false);
                      addToast('Currency: Indian Rupee (INR ₹) • Multi-tenant Indian GST enabled', 'info');
                    }}
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"
                  >
                    <div className="flex items-center gap-2.5">
      ...
      <Modal
        ...
        title="User Profile & Identity Credentials"
        description="DealFlow360 RBAC Account Verification"
        footer={
        ...
        <div className="space-y-4 text-xs text-left">
          <div className="flex items-center gap-3.5 p-3 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div>
              ...
              <p className="text-[11px] text-brand-600 font-mono mt-0.5">{getEmailForRole(currentUser.id)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl border border-slate-200 bg-white">
              ...
              <span className="font-mono font-bold text-slate-800 text-xs">
                DF360-EMP-{currentUser.id === 'sales_rep' ? '1048' : currentUser.id === 'sales_manager' ? '2019' : currentUser.id === 'finance_ops' ? '3081' : currentUser.id === 'customer' ? 'CUST-001' : '9901'}
              </span>
            </div>
            <div className="p-3 rounded-xl border border-slate-200 bg-white">
              <span className="text-[10px] uppercase font-bold text-slate-400 block">Location / Hub</span>
              <span className="font-bold text-slate-800 text-xs">{currentUser.id === 'customer' ? 'Mumbai HQ' : 'Bengaluru Tech Hub'}</span>
            </div>
            <div className="p-3 rounded-xl border border-slate-200 bg-white">
              <span className="text-[10px] uppercase font-bold text-slate-400 block">Currency & Locale</span>
              <span className="font-bold text-slate-800 text-xs">INR (₹) • en-IN</span>
            </div>
            <div className="p-3 rounded-xl border border-slate-200 bg-white">
              <span className="text-[10px] uppercase font-bold text-slate-400 block">Session Security</span>
          ...
          <div className="p-3 rounded-xl border border-slate-200 bg-slate-50">
            <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Key Responsibilities</span>
            <p className="text-slate-600 text-xs leading-relaxed">{currentUser.description}</p>
          </div>
        </div>
      </Modal>
    </header>
